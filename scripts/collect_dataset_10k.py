"""
10000件のbalanced Steam レビューデータセットを収集

Learning Curve実験用に10000件のデータセットを収集する。
- 目標: 5000 Positive + 5000 Negative = 10000件
- ゲーム: 7タイトル（既存と同じ）
- 各ゲーム: 約714 Positive + 714 Negative = 1428件
"""
...
def main():
    ...
    # 各ゲームから収集する件数（7ゲームで10000件）
    per_game_positive = 5000 // len(games)  # 714件/ゲーム
    per_game_negative = 5000 // len(games)  # 714件/ゲーム

    # 端数調整
    remainder = 5000 % len(games)

    all_positive_reviews = []
    all_negative_reviews = []

    print("=" * 60)
    print("Steam レビューデータセット収集開始（10000件）")
    print("=" * 60)
    print(f"\n収集対象: {len(games)}ゲーム")
    ...
    # 保存
    output_path = 'data/train/reviews_10000.csv'
