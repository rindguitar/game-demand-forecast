...
**着手順は #37 → #39 →（#40 相乗り）→ #38 → #41。** Phase 7（#41）は最後に回す。

理由: #37 は机上で決まり**捨てられない**（抽出の仕組みを変えても目標は変わらない）うえ、#39 と #38 の判断基準になる。#39 は再抽出（1回24分）を伴うので、目標が先に決まっていれば試行回数が減る。#40 はその再抽出に相乗りさせる。#41 はトピックの集合を**消費する側**なので、#39 で455トピックが入れ替わると作り直しになる。
